...
**Protocol status:** SafetyBench confirmatory component frozen as v1.0

**Model outputs examined:** Technical-validation and pilot outcomes only; confirmatory outcomes none
**Primary study type:** Paired observational evaluation of released checkpoints  
**Last updated:** 2026-08-31

The SafetyBench confirmatory component was frozen as v1.0 on 2026-08-31 before
the confirmatory sample was selected or scored. Its operational specification is
in [`confirmatory-preregistration.md`](confirmatory-preregistration.md). Any
later change must be logged with its date, reason, and whether it was made before
or after inspecting confirmatory outcome data.
...
- Use a fixed set of 70 SafetyBench items: 5 per category per language.
...
### Layer 3: SafetyBench confirmatory run

- Evaluate a frozen, balanced 4,200-item SafetyBench sample: 300 per category per language.
...
Test the omnibus null of no endpoint difference with Cochran's Q before
interpreting pairwise primary contrasts. Pairwise tests use exact McNemar tests.
...
## Deferred components and later protocol amendments

- Calibration, capability controls, and toxic-continuation evaluation are not
  part of the frozen SafetyBench accuracy component because the current scorer
  does not yet retain calibrated option probabilities and those measures require
  separate validation.
- Each deferred component requires its own preregistration before its outcomes
  are generated; it may not be retroactively added to this confirmatory family.
- The smallest effect size of interest, inference rules, exclusions, stopping
  rule, and failure policy are frozen in the confirmatory preregistration.
